...
# 数据集对应的label
PATH_TO_LABELS = os.path.join('faster_rcnn_data', 'mscoco_label_map.pbtxt')
NUM_CLASSES = 90
...
def process_image(image):
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            image_process, new_boxes = detect_objects(image, sess, detection_graph, category_index)
            return image_process, new_boxes
