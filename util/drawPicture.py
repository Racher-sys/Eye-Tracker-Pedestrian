...
from util.gazeplotter import draw_raw, draw_rect, draw_heatmap, draw_aoi, draw_aoi_pedestrain, draw_aoi_pedestrain2
...
def draw_rawpoint(data, path):
    ...
    for i in range(data.shape[0]):
        ...
        if tmp == mark:
            x1 = data.loc[i, 'GazePoint X']
            y1 = data.loc[i, 'GazePoint Y']
            x1 = max(x1 - width_adj, 0)
            y1 = max(y1 - height_adj, 0)
            x1 = min(x1, width_pic)
            y1 = min(y1, height_pic)
            x.append(x1)
            y.append(y1)
            ...
            # print("x:",x)
            # print("y:",y)
            draw_raw(x=x, y=y, dispsize=(768, 614), imagefile=backgroundImg, savefilename=path + str(tmp))
            x = []
            y = []
            tmp = mark
...
def judge(aoi, new_boxes):
    ...
    for i in range(len(aoi)):
        ...
        for j in range(len(new_boxes)):
            # w = (xmin+width/2-x)
            w = abs(new_boxes[j][0] + width_adj + new_boxes[j][2] / 2 - aoi[i][0])
            # h = (ymin+height/2-y)
            h = abs(new_boxes[j][1] + height_adj + new_boxes[j][3] / 2 - aoi[i][1])
    ...
    for i in range(data.shape[0]):
        ...
        if tmp == mark:
            ...
            if len(new_boxes) != 0:
                ...
                draw_aoi_pedestrain2(point1=F, point2=feature, box=box, allboxes=np.array(new_boxes),
                                     dispsize=(768, 614), imagefile=backgroundImg,
                                     savefilename=f"{path}/{str(tmp)}")

                # draw_target(box=box1, dispsize=(1280, 1024), imagefile=backgroundImg,
                #             savefilename=f"NewAssay/image/crop/hcy/{str(tmp)}.png")
...
def draw_fixation2(data, path):
    ...
    for i in range(data.shape[0]):
        ...
        if tmp == mark:
            ...
            feature = genterate_aoi(F, 65)
            # print(feature)
            draw_aoi(point1=F, point2=feature, dispsize=(768, 614), imagefile=backgroundImg,
                     savefilename=path + "source/" + str(tmp))

            draw_heatmap(gazepoints=F, dispsize=(768, 614), imagefile=backgroundImg,
                         savefilename=path + "heat1/" + str(tmp))
            draw_heatmap(gazepoints=F, dispsize=(768, 614), savefilename=path + "heat2/" + str(tmp))
