...
def draw_heatmap(gazepoints, dispsize, imagefile=None, alpha=0.5, savefilename=None, gaussianwh=200, gaussiansd=None):
    ...
    for i in range(0, len(gazepoints)):
        ...
        x1 = max(gazepoints[i][0] - width_adj, 0)
        y1 = max(gazepoints[i][1] - height_adj, 0)
        x1 = min(x1, width_pic)
        y1 = min(y1, height_pic)
        # get x and y coordinates
        x = int(strt + x1 - int(gwh / 2))
        y = int(strt + y1 - int(gwh / 2))
...
def draw_raw(x, y, dispsize, imagefile=None, savefilename=None):
    ...
    ax.plot(x, y, 'o', color='white', markeredgecolor=COLS['aluminium'][5])
...
def draw_aoi(point1, point2, dispsize, imagefile=None, savefilename=None):
    ...
    #     x1 = max(point1[i][0] - width_adj, 0)
    #     y1 = max(point1[i][1] - height_adj, 0)
    #     x1 = min(x1, width_pic)
    #     y1 = min(y1, height_pic)
    #     ax.plot(x1, y1, 'o', color=COLS['aluminium'][6], markeredgecolor=COLS['aluminium'][5],
    ...
    #     if point2[i][2] == 1:
    #         color = "red"
    #     x1 = max(point2[i][0] - width_adj, 0)
    #     y1 = max(point2[i][1] - height_adj, 0)
    #     x1 = min(x1, width_pic)
    #     y1 = min(y1, height_pic)
    #     ax.add_patch(
    #         patches.Rectangle((x1 - width / 2, y1 - height / 2), width, height, fill=False,
...
def draw_target(box, dispsize, imagefile=None, savefilename=None):
    """
    :param point1: raw fixation:[x,y,dur]
    :param point2: aio center:[x, y, last, count_fixation]
    :param box: [xmin, ymin, width, height]
    :param allboxes: [[xmin, ymin, width, height],..]
    :param imagefile: background image
    :param savefilename:
    :return: fig
    """
    ...
    x1 = np.maximum(int(box[1]), 0)
    x2 = np.minimum(int(box[1] + box[3]), X.shape[0] - 1)

    y1 = np.maximum(int(box[0]), 0)
    y2 = np.minimum(int(box[0] + box[2]), X.shape[1] - 1)
...
def draw_aoi_pedestrain(point1, point2, box, allboxes, dispsize, imagefile=None, savefilename=None):
    """
    :param point1: raw fixation:[x,y,dur]
    :param point2: aio center:[x, y, last, count_fixation]
    :param box: [xmin, ymin, width, height]
    ...
    for i in range(len(allboxes)):
        ax.add_patch(
            patches.Rectangle((allboxes[i][0] + width_adj, allboxes[i][1] + height_adj), allboxes[i][2], allboxes[i][3],
                              fill=False,
...
def draw_aoi_pedestrain2(point1, point2, box, allboxes, dispsize, imagefile=None, savefilename=None):
    """
    :param point1: raw fixation:[x,y,dur]
    :param point2: aio center:[x, y, last, count_fixation]
    :param box: [xmin, ymin, width, height]
    ...
    # for i in range(len(allboxes)):
    #     ax.add_patch(
    #         patches.Rectangle((allboxes[i][0], allboxes[i][1]), allboxes[i][2], allboxes[i][3],
    #                           fill=False,
    #                           color="pink", linewidth=2))
    ax.add_patch(patches.Rectangle((box[0], box[1]), box[2], box[3], fill=False, color='green', linewidth=4))
