...
```bash
# From tensorflow/models/research/
gsutil cp pet_faces_train.record-* gs://${YOUR_GCS_BUCKET}/faster_rcnn_data/
gsutil cp pet_faces_val.record-* gs://${YOUR_GCS_BUCKET}/faster_rcnn_data/
gsutil cp object_detection/faster_rcnn_data/pet_label_map.pbtxt gs://${YOUR_GCS_BUCKET}/faster_rcnn_data/pet_label_map.pbtxt
```
...
gcloud ml-engine jobs submit training `whoami`_object_detection_pets_`date +%m_%d_%Y_%H_%M_%S` \
    ...
    --pipeline_config_path=gs://${YOUR_GCS_BUCKET}/faster_rcnn_data/faster_rcnn_resnet101_pets.config
```
