...
def transform_input_data(tensor_dict,
                         ...
                         use_bfloat16=False):
  """A single function that is responsible for all input faster_rcnn_data transformations.
  ...
  Args:
    ...
    num_classes: number of max classes to one-hot (or k-hot) encode the class
      labels.
    data_augmentation_fn: (optional) faster_rcnn_data augmentation function to apply on
      input `tensor_dict`.
  ...
  # Apply faster_rcnn_data augmentation ops.
...
def pad_input_data_to_static_shapes(tensor_dict, max_num_boxes, num_classes,
  ...
  Args:
    tensor_dict: Tensor dictionary of input faster_rcnn_data
...
def augment_input_data(tensor_dict, data_augmentation_options):
  """Applies faster_rcnn_data augmentation ops to input tensors.
  ...
  Returns:
    A dictionary of tensors obtained by applying faster_rcnn_data augmentation ops to the
...
def create_train_input_fn(train_config, train_input_config,
  ...
  def _train_input_fn(params=None):
    ...
    Returns:
      A tf.faster_rcnn_data.Dataset that holds (features, labels) tuple.
