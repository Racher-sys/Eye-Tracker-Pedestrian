...
class BoxMaskList(np_box_list.BoxList):
  ...
  def __init__(self, box_data, mask_data):
    ...
    Raises:
      ...
      ValueError: if mask faster_rcnn_data is not a numpy array
      ValueError: if invalid dimension for mask faster_rcnn_data
    """
    super(BoxMaskList, self).__init__(box_data)
    if not isinstance(mask_data, np.ndarray):
      raise ValueError('Mask faster_rcnn_data must be a numpy array.')
    if len(mask_data.shape) != 3:
      raise ValueError('Invalid dimensions for mask faster_rcnn_data.')
    if mask_data.dtype != np.uint8:
      raise ValueError('Invalid faster_rcnn_data type for mask faster_rcnn_data: uint8 is required.')
