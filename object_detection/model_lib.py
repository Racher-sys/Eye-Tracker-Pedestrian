...
def _prepare_groundtruth_for_eval(detection_model, class_agnostic):
  """Extracts groundtruth faster_rcnn_data from detection_model and prepares it for eval.
...
def unstack_batch(tensor_dict, unpad_groundtruth_tensors=True):
  ...
  Note that this function has a static list of input faster_rcnn_data fields and has to be
  ...
  if unpad_groundtruth_tensors:
    ...
    unpad_keys = set([
        # List of input faster_rcnn_data fields that are padded along the num_boxes
...
def create_model_fn(detection_model_fn, configs, hparams, use_tpu=False):
  ...
  def model_fn(features, labels, mode, params=None):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      # For evaling on train faster_rcnn_data, it is necessary to check whether groundtruth
      # must be unpadded.
      boxes_shape = (
...
def create_estimator_and_inputs(run_config,
  ...
  Args:
    ...
    sample_1_of_n_eval_on_train_examples: Similar to
      ...
      faster_rcnn_data for evaluation.
...
def create_train_and_eval_specs(train_input_fn,
  ...
  Args:
    train_input_fn: Function that produces features and labels on train faster_rcnn_data.
    eval_input_fns: A list of functions that produce features and labels on eval
      faster_rcnn_data.
    eval_on_train_input_fn: Function that produces features and labels for
      evaluation on train faster_rcnn_data.
    predict_input_fn: Function that produces features for inference.
    train_steps: Number of training steps.
    eval_on_train_data: Whether to evaluate model on training faster_rcnn_data. Default is
      False.
  ...
  Returns:
    ...
    True, the last `EvalSpec` in the list will correspond to training faster_rcnn_data. The
